...
/* Controller 어노테이션
스프링 프레임워크에게 "이 클래스는 웨 브라우저의 요청(request)를
...
public class BookController {
	...
	public ModelAndView createPost(HttpServletRequest request, 
		...
		//title : 개똥이의 모험, category : 소설, category : 10000
	...
	public ModelAndView detail(BookVO bookVO, ModelAndView mav) {
		...
		//detail->bookVO(후) : BookVO [bookId=1, title=검은태양, category=드라마, price=10000, content=태양은 검은가?, insertDate=Wed Jul 19 16:27:36 KST 2023]
		log.info("detail->bookVO(후) : " + bookVO);
		
		
		
		mav.addObject("data", bookVO);
		//name="prefix" value="/WEB_INF/views/"
	...
	//요청파라미터 : BOOKVO [bookId=1, title=....]
	//요청방식 : post
	//파라미터는 String타입(bookId="1")->int bookId로 받으면 숫자로 자동형변환 됨
	...
	public ModelAndView deletePost(int bookId, String title, String category
		...
		// /detail?bookId=1
		if(result>0) {
			//redirect : URI재요청(목록으로 재요청 - 상세페이지 x)
			mav.setViewName("redirect:/list");
		}else {
			//삭제 실패 시 -> 책 목록페이지(list.jsp)로 이동
			// /list
			mav.setViewName("redirect:/detail?bookId="+bookVO.getBookId());
		}
