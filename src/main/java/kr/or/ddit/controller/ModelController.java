...
public class ModelController {
	...
	//		뷰(jsp)에서 참조할 이름을 클래스명(MemberVO)의 앞글자를 소문자(memberVO)로 변환하여 처리함
	@GetMapping("/read02")
	public String read02(Model model) {
		...
		//model.addAttribure("memberVO",memberVO);
		model.addAttribute(memberVO);
		
		return "model/read02";
	}
	
	//2-3)Model 객체에 자바빈즈 클래스 객체를 특정한 이름을 지정해서 전달할 수 있음
	//요청URI : /model/read03
	@GetMapping("/read03")
	...
	요청방식 : post
	 */
	//자바빈즈규칙(멤버변수, 기본생성자, getter/setter메소드)에 맞는 객체는
	// 매개변수로 선언하면 기본적으로 forwarding 시 데이터를 전달함
	@PostMapping("/register04")
	//@ModelAttribute를 써도 되고 안써도 됨
