...
public class BookInfoServiceImpl implements BookInfoService {
	 ...
	 * description=<p>내용</p> , name=개똥이의 여행, publisher=삼성출판사, releaseDate=2023-07-26
	...
	public int updateBookPost(BookInfoVO bookInfoVO) {
		...
		//2)첨부파일정보 수정
		//2-1)파일객체가 있음(파일도수정하겠음) bookInfoVO{...bookImage=org.springfra.......}
		if(bookInfoVO.getBookImage().getSize()>0) {
			...
			//2-2)파일객체가 없음(파일은수정안함) bookInfoVO{...bookImage=null}
			return result;
		}//end if
	}
	...
	public String getFolder() {
		...
		//간단한 날짜 형식
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		//날짜객체생성(java.util.Date)
		Date date = new Date();
		//2023-08-02
		String str = sdf.format(date);
		//File.separator : 윈도우 폴더 구분(2023\\08\\02)
